...
/** Canonical dark chat panel used by the Continue Beans entry point. */
public class ChatPanel extends JPanel {
    ...
    private final JTextField composer;
    ...
    public ChatPanel() {
        ...
        readyState = new JLabel("Ready. Describe what you want changed.");
        ...
        composer.addActionListener(event -> sendMessage());
        refresh.addActionListener(event -> llmClient.getModelosDisponiveisAsync());
    }

    private void sendMessage() {
        String message = composer.getText() == null ? "" : composer.getText().trim();
        if (processing || message.isEmpty()) {
            return;
        }

        composer.setText("");
        processing = true;
        llmClient.perguntarIAStreaming(
                "",
                message,
                ...
                chunk -> SwingUtilities.invokeLater(() -> {
                    if (chunk != null && !chunk.isEmpty()) {
                        conversation.append((conversation.getText().isEmpty() ? "" : "\n") + chunk);
                    }
                }),
                error -> SwingUtilities.invokeLater(() -> {
                    conversation.append("\nErro: " + (error == null ? "erro desconhecido" : error.getMessage()));
                    processing = false;
                }),
                () -> SwingUtilities.invokeLater(() -> processing = false));
    }
    ...
    public void clearChat() {
        ...
        readyState.setText("Ready. Describe what you want changed.");
        processing = false;
    }
}
