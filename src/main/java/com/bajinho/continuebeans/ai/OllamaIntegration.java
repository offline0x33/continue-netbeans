 ...
 * Uses text-based function calling for maximum compatibility.
 * 
 * @author Continue Beans Team
 */
public class OllamaIntegration {
    
    private static final Logger LOG = Logger.getLogger(OllamaIntegration.class.getName());
    ...
    public OllamaIntegration(String baseUrl, String model) {
    ...
    public CompletableFuture<String> processRequest(String userMessage) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                LOG.info("Processing request with Ollama: " + userMessage);
                
                // 1. Create enhanced system prompt
                JsonObject request = createOllamaRequest(userMessage);
                
                // 2. Send to Ollama
                JsonObject response = callOllamaAPI(request);
     ...
     */
    private JsonObject createOllamaRequest(String userMessage) {
     ...
     * Call Ollama API.
     */
    private JsonObject callOllamaAPI(JsonObject request) throws Exception {
    ...
    public CompletableFuture<Boolean> testConnection() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                JsonObject response = callOllamaAPI(testRequest);
